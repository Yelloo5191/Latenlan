...
class Ghoul(pygame.sprite.Sprite):
    ...
    def update(self, target):
        if self.rect.x == self.targetPosX or self.targetPosX < target.rect.x - 100 or self.targetPosX > target.rect.x + 100:
            self.targetPosX = random.randint(target.rect.x - 100, target.rect.x + 100)
        elif self.rect.x < self.targetPosX:
        ...
        if self.rect.y == self.targetPosY:
            self.targetPosY = random.randint(target.rect.y - 150, target.rect.y + 50)
        elif self.rect.y < self.targetPosY:
            self.y += self.speed
        elif self.rect.y > self.targetPosY:
            self.y -= self.speed
    ...
    def attack(self):
        ...
        if self.attack_timer == 0:
            for i in range(4):
                self.projectiles.append(Projectile(self.rect.x, self.rect.y, 1, 1, "assets/enemies/projectile.png", i * 90))
